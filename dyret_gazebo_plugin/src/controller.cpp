...
void GazeboDyretController::Load(gazebo::physics::ModelPtr model_,
                                 sdf::ElementPtr sdf_) {
    ...
    while (pid != nullptr) {
      ...
      if (pid->HasAttribute("type")) {
        ...
        ROS_WARN_STREAM("Could not use PID configuration '"
                        << pid->ToString("CFG") << "' because default is set");
      }
...
bool GazeboDyretController::InitROS() {
  ...
  auto reset_opts = ros::AdvertiseServiceOptions::create<std_srvs::SetBool>(
      ...
      std::bind(&GazeboDyretController::Reset, this, std::placeholders::_1,
                std::placeholders::_2),
      ros::VoidPtr(), &queue);
  ...
  auto state_opts = ros::AdvertiseOptions::create<dyret_common::State>(
      "/" + this->model->GetName() + "/state", 1, NULL, NULL, ros::VoidPtr(),
      &queue);
  ...
  auto pose_out_opts =
      ros::AdvertiseOptions::create<geometry_msgs::PoseStamped>(
          "/" + this->model->GetName() + "/sensor/pose", 1, NULL, NULL,
          ros::VoidPtr(), &queue);
  ...
  auto config_opts =
      ros::AdvertiseServiceOptions::create<dyret_common::Configure>(
  ...
  auto pose_opts = ros::SubscribeOptions::create<dyret_common::Pose>(
      "/" + this->model->GetName() + "/command", 5,
      std::bind(&GazeboDyretController::Pose, this, std::placeholders::_1),
      ros::VoidPtr(), &queue);
...
bool GazeboDyretController::Reset(const std_srvs::SetBool::Request &req,
      ...
      switch (cfg.type) {
        ...
        case dyret_common::RevoluteConfig::TYPE_SET_PID:
          if (cfg.parameters.size() != cfg.ids.size() * 3) {
            ...
            gazebo::common::PID pid(cfg.parameters[i * 3 + 0],
        ...
        case dyret_common::RevoluteConfig::TYPE_RESTART:
          ROS_WARN("Restart not supported in simulation");
          break;
        default:
      ...
      switch (cfg.type) {
        case dyret_common::PrismaticConfig::TYPE_SET_PID:
          if (cfg.parameters.size() < cfg.ids.size() * 3) {
            ...
            resp.status = dyret_common::Configure::Response::STATUS_PARAMETER;
            resp.message =
                "Expected at least three parameter per ID to set PID";
            return true;
          } else {
            gazebo::common::PID pid(cfg.parameters[i * 3 + 0],
                                    cfg.parameters[i * 3 + 1],
...
void GazeboDyretController::PublishState() {
  ...
  if (publishRate > 0 &&
      (currentTime - lastPublish).Double() < 1. / publishRate) {
    return;
  }
  ...
  if (mutex.try_lock()) {
    ...
    for (int i = 0; i < 12; ++i) {
      ...
      state.revolute[i].error =
          state.revolute[i].position - state.revolute[i].set_point;
    }
    for (int i = 0; i < 8; ++i) {
      ...
      state.prismatic[i].error =
          state.prismatic[i].position - state.prismatic[i].set_point;
    }
